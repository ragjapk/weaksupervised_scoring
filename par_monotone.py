...
class Network(nn.Module):
    def __init__(self,input_size1):
        ...
        self.weight34 = nn.Parameter(torch.Tensor(1,w_z3))

        
        self.reset_parameters()

    def reset_parameters(self):
        weights_1 = [self.weight11,self.weight12,self.weight13,self.weight14]
        for weight in weights_1:
            nn.init.xavier_uniform_(weight)
        weights_2 = [self.weight21, self.weight22, self.weight23, self.weight24]
        for weight in weights_2:
            nn.init.xavier_uniform_(weight)
        weights_3 = [self.weight31, self.weight32, self.weight33, self.weight34]
    ...
    def forward(self, x, y):
        ...
        z1 = torch.cat((x,y),1)
        z2 = nn.functional.linear(torch.mul(z1,nn.functional.linear(u1,self.weight21)),self.weight31)
        z2 = self.relu(z2)
        z3 = nn.functional.linear(torch.mul(z2,nn.functional.linear(u2,self.weight22)),self.weight32)
        z3 = self.relu(z3)
        z4 = nn.functional.linear(torch.mul(z3,nn.functional.linear(u3,self.weight23)),self.weight33)
        z4 = self.relu(z4)
        z5 = nn.functional.linear(torch.mul(z4,nn.functional.linear(u4,self.weight24)),self.weight34)
        return z4,z5
