...
class Network(nn.Module):
    def __init__(self,input_size):
        ...
        self.log_weight2 = nn.Parameter(torch.Tensor(256,500))
        self.log_weight3 = nn.Parameter(torch.Tensor(256,256))
        
        self.log_weight4 = nn.Parameter(torch.Tensor(1,256))
    ...
    def forward(self, x):
        ...
        pre_x = self.relu(x)
        x = nn.functional.linear(pre_x,self.log_weight4.exp())
        return pre_x,x
...
for epoch in range(num_epochs):
    for local_data in training_generator:
        ...
        temp_loss6 = torch.mean(torch.max(torch.zeros(batchsize,1),torch.abs((lower+5)*torch.ones(batchsize,1)-so)),0) 
        ...
        #kl_loss = -torch.log(torch.tensor(lamb)) + lamb*(mean1) - 0.5 - 0.5*torch.log(2*math.pi*dev1*dev1) - lamb
        ...
        if(args.dataset_name=='movie'):
            ...
            if(epoch<4000):
                loss = temp_loss9 + temp_loss7 + 100 * temp_loss6 + temp_loss1 + temp_loss2 + temp_loss3 + temp_loss4 + kl_loss
            else:
                loss = temp_loss1+temp_loss2+ 100*temp_loss9 + 100 * temp_loss6
        elif(args.dataset_name=='ads'):
            if(epoch<4000):
                loss = temp_loss9 + temp_loss7 + temp_loss1 + temp_loss2 + temp_loss3 + temp_loss4 + kl_loss
            else:
                loss = kl_loss + temp_loss7 + temp_loss9
            #- 0.1*temp_loss5 
            #loss = kl_loss'''
