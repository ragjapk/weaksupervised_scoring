...
if args.dataset_name == 'movie':
    ...
    lower = 40
    lamb = 0.5
elif args.dataset_name == '_movie':
    df = pd.read_csv(args.data_path+'data_11_movie.csv', encoding="utf-8",sep=',', header=None)
    df = df.drop(df.columns[-1],axis=1)
    min_ = [3]
    learning_rate = 0.0005
    num_epochs = 2000
...
data2['xtrain'] =X_train_total[ind,1:]
data2['ytrain'] = y_train[ind]
...
class Network(nn.Module):
    def __init__(self,input_size):
        ...
        self.log_weight2 = nn.Parameter(torch.Tensor(1000,500))
        self.log_weight3 = nn.Parameter(torch.Tensor(256,1000))
...
for epoch in range(num_epochs):
    ...
    for local_data in training_generator:
        ...
        if(args.dataset_name=='movie'):
            loss =  temp_loss5  + 0.1 * temp_loss1
        elif(args.dataset_name=='college'):
            if(epoch > 8000 and epoch < 10000):
                loss = temp_loss6  + temp_loss9 +temp_loss7 
            elif(epoch>10000):
                loss = temp_loss92 + temp_loss7 + temp_loss6 + temp_loss1 + temp_loss2 + temp_loss3 + temp_loss4
                #+ quant_loss1
            #- 0.1*temp_loss5 
            #loss = kl_loss'''
        elif(args.dataset_name=='journal'):
...
print('***************')
csv_file = np.hstack((X_test_total,predicted.reshape(-1,1)))
