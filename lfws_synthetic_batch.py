...
test_size = 0.2
val_size = 0.1
X_train_val, X_test_total, y_train_val, y_test = train_test_split(X_total, y, test_size=0.2, random_state=42)
relative_train_size = train_size / (val_size + train_size)
X_train_total, X_val_total, y_train, y_val = train_test_split(X_train_val, y_train_val, train_size = relative_train_size, test_size = 1-relative_train_size,random_state=42)

X = X_test_total

data = dict()
data['train'] =X_train_total[:,:]
data['validation']= X_val_total[:,:]
data['test']= X_test_total[:,:]

params = {'batch_size': args.batch_size,
          'shuffle': True}

X_test_total = Variable(torch.from_numpy(X_test_total.astype(np.float32)))
X_test_total = X_test_total.to(device)

X_val_total = Variable(torch.from_numpy(X_val_total.astype(np.float32)))
X_val_total = X_val_total.to(device)

training_set = Dataset(data['train'])
...
for epoch in range(num_epochs):
    for local_data in training_generator:
        ...
        #Code to ensure a monotonic network
        #with torch.no_grad():
