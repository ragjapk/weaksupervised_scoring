...
class Network(nn.Module):
    def __init__(self,input_size):
        ...
        self.log_weight3 = nn.Parameter(torch.Tensor(1,500))
...
for epoch in range(num_epochs):
    for local_data in training_generator:
        ...
        if(args.dataset_name=='ads'):
            ...
            for i in range(features):
                temp_loss2+= mean_grad[i]/(10*mean_grad[1]+mean_grad[0])
        temp = so-upper*torch.ones(batchsize,1)
        temp_loss7 = torch.mean(torch.max(torch.zeros(batchsize,1),torch.sign(temp)*torch.square(temp)),0)
        temp_loss72 = torch.mean(torch.max(torch.zeros(batchsize,1),torch.exp(temp)),0)
        temp_loss6 = torch.mean(torch.max(torch.zeros(batchsize,1),torch.abs(lower+2*torch.ones(batchsize,1)-so)),0) 
        ...
        temp_loss9 = torch.mean(torch.max(torch.zeros(batchsize,1),torch.sign(temp)*torch.square(temp)),0)
        dev1 = torch.std(so)
        mean1 = torch.mean(so)
        ...
        kl_loss = -torch.log(torch.tensor(lamb)) + lamb*mean1 - 0.5 - 0.5*torch.log(2*math.pi*dev1*dev1) - lamb*10
        
        
        if(args.dataset_name=='movie'):
            loss = torch.max(torch.zeros(1,1),kl_loss) + temp_loss5  + 0.1 * temp_loss1
        elif(args.dataset_name=='journal'):
            #loss = 0.1*torch.max(torch.zeros(1,1),kl_loss) + temp_loss6 + 10*temp_loss7 + temp_loss9 +temp_loss1 + temp_loss2 + temp_loss3 + temp_loss4 
            #loss = temp_loss9 +  temp_loss8 + temp_loss1 + temp_loss6 + temp_loss2+temp_loss3 + temp_loss4
            if epoch<5000:
                loss = temp_loss6  + temp_loss9 +temp_loss7
            else:
                loss = 2*temp_loss92 + temp_loss6 + temp_loss8 + (2*temp_loss1/temp_loss2) + (2*temp_loss2/temp_loss3) +temp_loss1/temp_loss3 + temp_loss7  
            #- 0.1*temp_loss5 
            #loss = kl_loss'''
