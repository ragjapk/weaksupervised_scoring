...
if args.dataset_name == 'movie':
    ...
    df = pd.read_csv(args.data_path+'train_college.csv', encoding="utf-8",sep=',', header=None)
    df2 = pd.read_csv(args.data_path+'test_college.csv', encoding="utf-8",sep=',', header=None)
    min_ = []
    learning_rate = 0.001
    num_epochs = 3000
    upper = 100
...
X_train_total, X_test_total, y_train, y_test = train_test_split(X_total, y, test_size=0.3)

sup_indices_sm = np.argpartition(y_train,32)[:32]
sup_indices_la = np.argpartition(y_train,-32)[-32:]
ind = np.concatenate([sup_indices_sm,sup_indices_la])
...
X = X.to(device)
#x_test = Variable(torch.from_numpy(X_test_total[:,0:].astype(np.float32)))
...
class Network(nn.Module):
    def __init__(self,input_size1):
        ...
        self.weight11 = nn.Parameter(torch.Tensor(w_u1, input_size1))
        self.weight12 = nn.Parameter(torch.Tensor(w_u2,w_u1))
        self.weight13 = nn.Parameter(torch.Tensor(w_u3,w_u2))
        self.weight14 = nn.Parameter(torch.Tensor(w_u4,w_u3))
        self.weight15 = nn.Parameter(torch.Tensor(w_u5,w_u4))
        
        self.weight21 = nn.Parameter(torch.Tensor(input_size2+input_size1,w_u1))
        self.weight22 = nn.Parameter(torch.Tensor(w_z1,w_u2))
        self.weight23 = nn.Parameter(torch.Tensor(w_z2,w_u3))
        self.weight24 = nn.Parameter(torch.Tensor(w_z3,w_u4))
        self.weight25 = nn.Parameter(torch.Tensor(w_z4,w_u5))
        
        self.weight31 = nn.Parameter(torch.Tensor(w_z1,input_size2+input_size1))
        self.weight32 = nn.Parameter(torch.Tensor(w_z2,w_z1))
        self.weight33 = nn.Parameter(torch.Tensor(w_z3,w_z2))
        self.weight34 = nn.Parameter(torch.Tensor(w_z4,w_z3))
        self.weight35 = nn.Parameter(torch.Tensor(1,w_z4))
        
        self.reset_parameters()

    def reset_parameters(self):
        weights_1 = [self.weight11,self.weight12,self.weight13,self.weight14,self.weight15]
        for weight in weights_1:
            nn.init.xavier_uniform_(weight)
        weights_2 = [self.weight21, self.weight22, self.weight23, self.weight24, self.weight25]
        for weight in weights_2:
            nn.init.xavier_uniform_(weight)
        weights_3 = [self.weight31, self.weight32, self.weight33, self.weight34, self.weight35]
        for weight in weights_3:
            nn.init.xavier_uniform_(weight)
    
    def forward(self, x, y):
        # Pass the input tensor through each of our operations
        u0 = x
        u1 = nn.functional.linear(u0,self.weight11)
        u1 = self.relu(u1)
        u2 = nn.functional.linear(u1,self.weight12)
        u2 = self.relu(u2)
        u3 = nn.functional.linear(u2,self.weight13)
        u3 = self.relu(u3)
        u4 = nn.functional.linear(u3,self.weight14)
        u4 = self.relu(u4)
        u5 = nn.functional.linear(u4,self.weight15)
        u5 = self.relu(u5)
        
        
        z1 = torch.cat((x,y),1)
        z2 = nn.functional.linear(torch.mul(z1,nn.functional.linear(u1,self.weight21)),self.weight31,bias=False)
        z2 = self.relu(z2)
        z3 = nn.functional.linear(torch.mul(z2,nn.functional.linear(u2,self.weight22)),self.weight32,bias=False)
        z3 = self.relu(z3)
        z4 = nn.functional.linear(torch.mul(z3,nn.functional.linear(u3,self.weight23)),self.weight33,bias=False)
        z4 = self.relu(z4)
        z5 = nn.functional.linear(torch.mul(z4,nn.functional.linear(u4,self.weight24)),self.weight34,bias=False)
        z5 = self.relu(z5)
        z6 = nn.functional.linear(torch.mul(z5,nn.functional.linear(u5,self.weight25)),self.weight35,bias=False)        
        return z5,z6

    
for batch,labels in training_generator2:
...
for epoch in range(num_epochs):
    for sup_batch, sup_label in training_generator2:
        sup_batch, sup_label = sup_batch.to(device), sup_label.to(device)
        act1,pred = model(sup_batch[:,[0,1,2,3,8]],sup_batch[:,[4,5,6,7]])
        sup_loss = l1(pred,sup_label)
        ...
        temp = so-upper*torch.ones(batchsize,1)
        temp_loss7 = torch.mean(torch.max(torch.zeros(batchsize,1),temp),0)
        temp_loss72 = torch.mean(torch.max(torch.zeros(batchsize,1),torch.exp(temp)),0)
        temp_loss6 = torch.mean(torch.max(torch.zeros(batchsize,1),torch.square(((lower+5)*torch.ones(batchsize,1))-so)),0) 
        #temp_loss8 = torch.mean(torch.max(torch.zeros(batchsize,1),torch.abs(upper*torch.ones(batchsize,1)-so)),0)
        temp = lower*torch.ones(batchsize,1)-so
        temp_loss9 = torch.mean(torch.max(torch.zeros(batchsize,1),temp),0)
        ...
        if(args.dataset_name=='movie'):
            loss =  temp_loss5  + temp_loss1 + temp_loss9 + temp_loss6 + temp_loss7 + temp_loss9
        elif(args.dataset_name=='_college'):
            if(epoch <= 5000):
                loss = temp_loss6  + temp_loss9 +temp_loss7 
            elif(epoch>5000):
                loss = temp_loss9 + temp_loss7 + temp_loss6 + temp_loss1 + temp_loss2 + temp_loss3 + temp_loss4
                #+ quant_loss1
            #- 0.1*temp_loss5 
            #loss = kl_loss
        elif(args.dataset_name=='journal'):
            if epoch<5000:
                loss = 10*temp_loss6  + temp_loss9 +temp_loss7 
            else:
                loss =  2*temp_loss9 + temp_loss6  + (2*temp_loss1/temp_loss2) + (2*temp_loss2/temp_loss3) +temp_loss1/temp_loss3 + temp_loss7  
    ...
    if(sup_loss<c_loss):
        c_loss = sup_loss
        act1,pred = model(X[:,[0,1,2,3,8]],X[:,[4,5,6,7]])
