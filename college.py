...
if(args.dataset_name=='college'):
    float_arr2 = df2.values.copy()
    X_train_total = float_arr[:,:-1]
    X_test_total =  float_arr2[:,:-1]
    y_train = float_arr[:,-1]
    y_test =  float_arr2[:,-1]
else:
    X_total=float_arr[:,:-1]
    ...
    X_train_total, X_test_total, y_train, y_test = train_test_split(X_total, y, test_size=0.3, random_state = 76)
...
class Network(nn.Module):
    ...
    '''def forward(self, x):
        ...
        x = self.relu(x)
        x = nn.functional.linear(x,self.log_weight4.exp())
        pre_x = self.relu(x)
        x = nn.functional.linear(pre_x,self.log_weight5.exp())
        return pre_x,x'''
